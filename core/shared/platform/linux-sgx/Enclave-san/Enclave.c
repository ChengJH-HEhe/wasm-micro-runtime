...
typedef enum EcallCmd {
    ...
    CMD_LOOKUP_WASI_START_FUNCTION, 
    CMD_COPY_POINTER,
    CMD_COPY_TO_WASM,
} EcallCmd;
...
//static char global_heap_buf[10 * 1024 * 1024] __attribute__((section(".wasm.heap"))) __attribute__((aligned(64)));// = { 0 };
//#else
char global_heap_buf[_WASM_HEAP_SIZE] __attribute__((section(".wasm.heap"))) __attribute__((aligned(64)));// = { 0 };
//#endif
...
handle_cmd_lookup_function(uint64 *args, uint32 argc)
{
    ...
    module_inst = (wasm_module_inst_t)(args[0]);
    name = (char *)(args[1]);
    signature = (char *)(args[2]);
...
handle_cmd_copy_pointer(uint64 *args, uint32 argc)
{
    ...
    module_inst = (wasm_module_inst_t )(args[0]);
    app_offset = (uint32)(args[1]);
    len = (size_t)(args[2]);
    external_buffer = (void *)(args[3]);

    internal_buffer = wasm_runtime_addr_app_to_native(module_inst, app_offset);
...
handle_cmd_call_wasm_a(uint64 *args, uint32 argc)
{
    ...
    exec_env = (wasm_exec_env_t )(args[0]);
    function = (wasm_function_inst_t )(args[1]);
    num_results = (uint32)(args[2]);
    results = (wasm_val_t *)(args[3]);
    num_args = (uint32)(args[4]);
    wasm_args = (wasm_val_t *)(args[5]);
...
handle_cmd_init_runtime(uint64 *args, uint32 argc)
{
    ...
    //snprintf(outside_print_buffer, 99, ".data @ 0x%016lX\n", (uint64_t)&__data_start);
    //ocall_print(outside_print_buffer);
    //snprintf(outside_print_buffer, 99, ".data ends @ 0x%016lX\n", (uint64_t)&_edata);
    //ocall_print(outside_print_buffer);
    ...
    if (alloc_with_pool) {
        ...
        printf("heap_buf = 0x%016llX\n", (uint64_t)global_heap_buf);
...
extern uint32_t last_session_id;
 int decrypt_wasm_file(void* req_message,
...
handle_cmd_load_module(uint64 *args, uint32 argc)
{
    ...
    char *wasm_file;
    uint32 wasm_file_size_encrypted = *(uint32 *)args++;
    uint32 wasm_file_size;
    ...
    printf("wasm_file_size = %lld\n", (uint64_t)wasm_file_size);
    printf("wasm_file = 0x%016llX\n", (uint64_t)wasm_file);
...
handle_cmd_set_wasi_args(uint64 *args, int32 argc)
{
    ...
    uint32 dir_list_size = (uint32 )(args[2]);
    char **env_list = (char **)(args[3]);
    uint32 env_list_size = (uint32 )(args[4]);
    int stdinfd = -1;
    int stdoutfd = -1;
    int stderrfd = -1;
    char **wasi_argv = (char **)(args[8]);
    char *p, *p1;
    uint32 wasi_argc = (uint32)(args[9]);
    uint64 total_size = 0;
    int32 i, str_len;
                     ...
                     unsigned cmd_buf_size)
{
    ...
    switch (cmd) {
        ...
        case CMD_LOOKUP_FUNCTION:
            handle_cmd_lookup_function(args, argc);
            break;
        case CMD_CREATE_EXEC_ENV:
            handle_cmd_create_exec_env(args, argc);
            break;
        case CMD_CALL_WASM_A:
            handle_cmd_call_wasm_a(args, argc);
            break;
        ...
        case CMD_SET_LOG_LEVEL:
            ...
            break;
        case CMD_DESTROY_EXEC_ENV:
            handle_cmd_destroy_exec_env(args, argc);
            break;
        case CMD_LOOKUP_WASI_START_FUNCTION:
            handle_cmd_lookup_wasi_start_function(args, argc);
            break;
        case CMD_COPY_POINTER:
            handle_cmd_copy_pointer(args, argc);
            break;
        case CMD_COPY_TO_WASM:
            handle_cmd_copy_to_wasm(args, argc);
            break;
        default:
